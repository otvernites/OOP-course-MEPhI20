...
namespace Accs {
	...
	protected:
		...
		void bypass(Node<K, P>* ptr); // для удаления (лев-прав-корень)
		void bypass_direct(Node<K, P>* ptr); // для копирования
	public:
		My_Map() : root(nullptr) {}
	...
	template <class K, class P>
	void My_Map<K, P>::bypass_direct(Node<K, P>* ptr) {
		if (!ptr) { return; }
		MyPair<K, P>* pair_copy = new MyPair<K,P>(*(ptr->info));
		My_Map<K, P>::insert(pair_copy);
		My_Map<K, P>::bypass_direct(ptr->left);
		My_Map<K, P>::bypass_direct(ptr->rigth);
	}

	template <class K, class P>
	My_Map<K, P>::My_Map(const My_Map<K, P>& map) {
		My_Map<K, P>::bypass_direct(map.root);
	}
	...
	My_Map<K, P>& My_Map<K, P>::operator = (const My_Map<K, P>& second) {
		...
		My_Map<K, P>::bypass_direct(second.root);
		return *this;
	}
	template <class K, class P>
